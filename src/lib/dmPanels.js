  ...
  return {
    embeds: [
      new EmbedBuilder()
        ...
        .addFields(
          { name: 'Vencimento', value: dueAt || 'nao informado', inline: true },
          { name: 'Canal do projeto', value: channelId ? `<#${channelId}>` : 'nao informado', inline: true }
        )
        .setFooter({ text: 'O acesso pode ser suspenso apos o prazo de tolerancia.' })
        .setTimestamp()
    ],
    components: [
      new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`hosting_paid:${channelId}`).setLabel('Ja paguei').setStyle(ButtonStyle.Success),
        new ButtonBuilder().setCustomId(`hosting_unpaid:${channelId}`).setLabel('Nao paguei').setStyle(ButtonStyle.Danger)
      )
    ]
  };
}
